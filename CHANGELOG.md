...
- **Borrow on Lighter reads as Gate charges it.** A Lighter USDC borrow pays about 11% a year from
  ...
  wallet.
- **Spot loop stops at 15 min.** Convert always shows. A Spot loop row shows only when it costs
  less than Convert. It runs the rounds that fit in 15 min, then Convert does the rest. A loop of
  100 rounds and 12 hours no longer shows.
