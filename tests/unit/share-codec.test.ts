...
import {
  ...
  roundTo,
  type SharePayloadV1,
} from '../../src/core/share/codec';

// The same case list runs against the byte-identical browser mirror in
// web/src/lib/shareCodec.test.ts — keep the two suites and the golden vector
 ...
 * web/src/lib/shareCodec.test.ts. */
...
describe('decodeSharePayload', () => {
  ...
  it("reports 'version' for a well-formed payload from a newer schema", () => {
    ...
    // A non-numeric v is not a version signal, just garbage.
  ...
  it('rejects unknown keys anywhere (including __proto__) without polluting prototypes', () => {
    ...
    const proto = b64url(GOLDEN_JSON_WITH('"__proto__":{"polluted":true},'));
...
/** Splice extra JSON just after `{` of the golden fixture's JSON. */
function GOLDEN_JSON_WITH(insert: string): string {
  const json = JSON.stringify(fixture);
  return `{${insert}${json.slice(1)}`;
}
